...
async def _predictor_retention_loop():
    ...
    cycle_accuracy) — righe più vecchie di
...
def partial_ist_table(request: Request):
    ...
    for ist in insts:
        ...
        # WU211 — livello edificio di trasporto (rifornimento)
        livello_trasporto = ist_ov.get("livello_trasporto", ist.get("livello_trasporto", 20))
        ...
        rows.append(f'''<tr data-nome="{nome}" class="ist-row {'disabled' if not abilitata else ''}">
          ...
          <td><input type="number" class="ist-lv" value="{livello}" {disabled_attr}
                     min="1" max="10" style="width:36px"></td>
          <td><input type="number" class="ist-trasporto" value="{livello_trasporto}" {disabled_attr}
                     min="1" max="25" title="livello edificio di trasporto (1-25)" style="width:40px"></td>
