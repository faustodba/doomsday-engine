...
# ──────────────────────────────────────────────────────────────────────────────
# AB TEST ADAPTIVE SCHEDULER
# ──────────────────────────────────────────────────────────────────────────────

@app.get("/ui/ab-test", include_in_schema=False)
def ui_ab_test(request: Request):
    """Pagina dedicata AB test adaptive scheduler vs naive."""
    return templates.TemplateResponse(request, "ab_test.html", {
        "active": "ab_test",
        **_env_label(),
    })


def _load_ab_records() -> list[dict]:
    """Carica tutti i record da scheduler_ab.jsonl. Ritorna lista vuota se assente."""
    import json as _json
    p = Path("C:/doomsday-engine-prod") / "data" / "predictions" / "scheduler_ab.jsonl"
    if not p.exists():
        return []
    out = []
    try:
...
def partial_ab_test_summary(request: Request):
    ...
    records = _load_ab_records()
    if not records:
        return HTMLResponse("""
        <div class='tel-grid'>
          <div class='tel-card tel-full'>
            <div class='tel-head'>riepilogo ab test</div>
            <div style='padding:14px;color:var(--text-dim);font-size:12px'>
              nessun dato disponibile — scheduler non ancora attivato
            </div>
          </div>
        </div>""")

    n       = len(records)
    deltas  = [r.get("delta_slot", 0) for r in records]
    vince   = sum(1 for d in deltas if d > 0)
    tie     = sum(1 for d in deltas if d == 0)
    perde   = sum(1 for d in deltas if d < 0)
    media   = sum(deltas) / n
    best    = max(deltas)
    worst   = min(deltas)
    pct_win = vince / n * 100

    # barra progresso win rate
    bar_w   = round(pct_win)
    bar_col = "var(--accent)" if pct_win >= 70 else "#e07820" if pct_win >= 50 else "#c0392b"

    # trigger più frequenti
    from collections import Counter
    reasons_flat = []
    for r in records:
        for x in (r.get("reasons") or ["none"]):
            reasons_flat.append(x.split("=")[0])
    top_reasons = Counter(reasons_flat).most_common(3)
    reasons_str = " · ".join(f"{k} {v}×" for k, v in top_reasons)

    html = f"""
    <div class='tel-grid'>
      <div class='tel-card tel-full'>
        <div class='tel-head'>riepilogo ab test · {n} cicli registrati</div>
        <div style='padding:14px'>

          <div style='display:flex;gap:32px;flex-wrap:wrap;margin-bottom:16px'>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:var(--accent)'>{media:+.2f}</div>
              <div style='font-size:11px;color:var(--text-dim)'>delta medio slot/ciclo</div>
            </div>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:#4caf50'>{best:+.0f}</div>
              <div style='font-size:11px;color:var(--text-dim)'>best case</div>
            </div>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:#e57373'>{worst:+.0f}</div>
              <div style='font-size:11px;color:var(--text-dim)'>worst case</div>
            </div>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:var(--fg)'>{vince}/{n}</div>
              <div style='font-size:11px;color:var(--text-dim)'>adaptive vince</div>
            </div>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:var(--text-dim)'>{tie}/{n}</div>
              <div style='font-size:11px;color:var(--text-dim)'>tie</div>
            </div>
            <div style='text-align:center'>
              <div style='font-size:28px;font-weight:700;color:#e57373'>{perde}/{n}</div>
              <div style='font-size:11px;color:var(--text-dim)'>naive vince</div>
            </div>
          </div>
          ...
          <div style='background:var(--bg2);border-radius:4px;height:12px;width:100%;margin-bottom:16px'>
            <div style='background:{bar_col};width:{bar_w}%;height:12px;border-radius:4px;transition:width 0.5s'></div>
          </div>

          <div style='font-size:12px;color:var(--text-dim)'>
            trigger attivazione: {reasons_str}
          </div>
        </div>
        <div class='tel-foot'>
          adaptive scheduler vince quando delta_slot &gt; 0 · tie = ordine identico · naive = alfabetico FAU_00→FAU_10
        </div>
      </div>
    </div>"""
...
@app.get("/ui/partial/ab-test-trend", include_in_schema=False)
def partial_ab_test_trend(request: Request):
    """SVG trend delta_slot per ciclo."""
    records = _load_ab_records()
    if not records:
        return HTMLResponse("""
        <div class='tel-grid'>
          <div class='tel-card tel-full'>
            <div class='tel-head'>trend delta_slot</div>
            <div style='padding:14px;color:var(--text-dim);font-size:12px'>nessun dato</div>
          </div>
        </div>""")
    ...
    d_min  = min(min(deltas), -1)
    d_max  = max(max(deltas),  1)
    ...
    def sy(v):
        return pad_t + (1 - (v - d_min) / d_range) * plot_h

    # linea zero
    y0     = sy(0)
    points = " ".join(f"{sx(i):.1f},{sy(d):.1f}" for i, d in enumerate(deltas))

    # cerchi colorati
    circles = ""
    for i, d in enumerate(deltas):
        col = "#4caf50" if d > 0 else "#e57373" if d < 0 else "#888"
        circles += (
            f"<circle cx='{sx(i):.1f}' cy='{sy(d):.1f}' r='4' "
            f"fill='{col}' stroke='var(--bg)' stroke-width='1'>"
            f"<title>ciclo {i+1}: delta={d:+.0f}</title></circle>"
        )

    # etichette asse Y
    y_labels = ""
    for v in [d_min, 0, d_max]:
        y_labels += (
            f"<text x='{pad_l - 6}' y='{sy(v) + 4:.1f}' "
            f"text-anchor='end' font-size='10' fill='var(--text-dim)'>{v:+.0f}</text>"
        )
    ...
    <svg viewBox='0 0 {W} {H}' xmlns='http://www.w3.org/2000/svg'
      ...
      <!-- linea zero -->
      <line x1='{pad_l}' y1='{y0:.1f}' x2='{pad_l+plot_w}' y2='{y0:.1f}'
            stroke='var(--text-dim)' stroke-width='1' stroke-dasharray='4,3'/>
      <text x='{pad_l + plot_w + 4}' y='{y0 + 4:.1f}'
            font-size='10' fill='var(--text-dim)'>naive</text>
      <!-- linea trend -->
      ...
      <text x='{W/2}' y='{H-4}' text-anchor='middle'
            font-size='10' fill='var(--text-dim)'>cicli (cronologico →)</text>
    </svg>"""

    html = f"""
    <div class='tel-grid'>
      <div class='tel-card tel-full'>
        <div class='tel-head'>trend delta_slot · {n} cicli · verde=adaptive vince · rosso=naive vince</div>
        <div style='padding:14px'>{svg}</div>
        <div class='tel-foot'>
          ogni punto = 1 ciclo AB test · linea tratteggiata = baseline naive (delta=0)
        </div>
      </div>
    </div>"""
    return HTMLResponse(html)
...
@app.get("/ui/partial/ab-test-detail", include_in_schema=False)
def partial_ab_test_detail(request: Request):
    """Tabella ultimi 15 cicli AB test."""
    records = _load_ab_records()
    if not records:
        return HTMLResponse("""
        <div class='tel-grid'>
          <div class='tel-card tel-full'>
            <div class='tel-head'>dettaglio cicli</div>
            <div style='padding:14px;color:var(--text-dim);font-size:12px'>nessun dato</div>
          </div>
        </div>""")

    ultimi  = records[-15:][::-1]   # ultimi 15, dal più recente
    ...
    for r in ultimi:
        ...
        delta   = r.get("delta_slot", 0)
        adp     = r.get("tot_adaptive", 0)
        naive   = r.get("tot_naive", 0)
        n_ist   = r.get("n_istanze", "—")
        reasons = " · ".join(r.get("reasons") or ["—"])
        reasons = reasons[:40] + "…" if len(reasons) > 40 else reasons

        if delta > 0:
            delta_html = f"<span style='color:#4caf50;font-weight:600'>{delta:+.0f} ✅</span>"
        elif delta < 0:
            delta_html = f"<span style='color:#e57373;font-weight:600'>{delta:+.0f} ❌</span>"
        else:
            delta_html = f"<span style='color:var(--text-dim)'>{delta:+.0f} —</span>"

        ordine  = r.get("ordine_adaptive") or []
        ordine_str = " → ".join(ordine[:5])
        if len(ordine) > 5:
            ordine_str += f" +{len(ordine)-5}"

        rows += f"""
        <tr>
          <td style='color:var(--text-dim)'>{ts_str}</td>
          <td style='font-size:11px'>{reasons}</td>
          <td style='text-align:center'>{n_ist}</td>
          <td style='text-align:center;color:var(--accent)'>{adp:.0f}</td>
          <td style='text-align:center;color:var(--text-dim)'>{naive:.0f}</td>
          <td style='text-align:center'>{delta_html}</td>
          <td style='font-size:11px;color:var(--text-dim)'>{ordine_str}</td>
        </tr>"""

    html = f"""
    <div class='tel-grid'>
      <div class='tel-card tel-full'>
        <div class='tel-head'>ultimi {len(ultimi)} cicli · dal più recente</div>
        <div style='padding:14px;overflow-x:auto'>
          <table style='width:100%;border-collapse:collapse;font-size:12px'>
            <thead>
              <tr style='color:var(--text-dim);border-bottom:1px solid var(--border)'>
                <th style='text-align:left;padding:4px 8px'>ora</th>
                <th style='text-align:left;padding:4px 8px'>trigger</th>
                <th style='text-align:center;padding:4px 8px'>istanze</th>
                <th style='text-align:center;padding:4px 8px'>adaptive Σ</th>
                <th style='text-align:center;padding:4px 8px'>naive Σ</th>
                <th style='text-align:center;padding:4px 8px'>delta</th>
                <th style='text-align:left;padding:4px 8px'>ordine adaptive</th>
              </tr>
            </thead>
            <tbody>
              {rows}
        ...
        <div class='tel-foot'>
          ultimi 15 record · adaptive Σ = somma slot_liberi_atteso ordine adattivo · naive Σ = ordine alfabetico
        </div>
      </div>
    </div>"""
    return HTMLResponse(html)
...
def partial_predictor_distribuzione(request: Request):
    ...
    return HTMLResponse(
        ...
        '<th style="text-align:right">Δt p25</th>'
        '<th style="text-align:right">Δt p50</th>'
        '<th style="text-align:right">Δt p75</th>'
        '<th>raccom.</th>'
