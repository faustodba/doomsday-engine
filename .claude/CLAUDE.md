...
| raccolta | WU199 | report_raccolta — lettura Gathering Report per produzione/ora precisa | MEDIA | 🟡 FASE 2 ATTIVA IN PROD 10/07/2026. Piano 3 fasi: (1) reset ✅, (2) lettura+storage+scroll ✅ attiva su tutte le 12 istanze (232+ righe reali in `data/report_raccolta_dataset.jsonl`), (3) sostituzione algoritmo produzione — non iniziata. WU199quinquies: sanity check capacità nominale (`_CAPACITA_MAX`) contro bleed OCR icona risorsa (cifra spuria prependuta, es. campo→"5", segheria→"2"). Dettagli completi in `docs/issues/raccolta.md` |
