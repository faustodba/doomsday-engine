...
| ocr-vision | 54 | Banner catalog & dismissal pipeline boot stabilization — 573 UNKNOWN polls | — | 🟡 parziale — framework + 4 banner attivi (exit_game_dialog, auto_collect_afk_banner, equipment_report, privileges_subscription_trial WU237 22/07); banner_eventi_laterale resta disabilitato (26/04, conflitto icona DS) |
...
| telemetria-predictor | WU224-229 | Pannello report-raccolta + TTL + doppio giro nel piano + log rotation | MEDIA | ✅ CHIUSA 15-16/07. Catena partita da una richiesta sul pannello in-volo, ogni fix ha fatto emergere il successivo: **WU225** TTL orfane 4h→12h (potava prima che il report del completamento venisse letto — esiste solo al boot successivo dell'istanza; recupero 41%→~100% dei campioni) + ricostruzione storico `tools/rebuild_tempo_raccolta_dataset.py` (491→1208 match). **WU226** riclassificazione stati pannello (orfana/in-volo/attesa-lettura — il "ritardo" non è un concetto valido: la stima è una mediana) + esclusione master FauMorfeus (giocato a mano) dai report. **WU227** rotazione log `.jsonl` (fix, vedi `infra-startup.md`). **WU228/228b** doppio giro FAU_00 visibile nella pianificazione (voce virtuale opt-in, soppressa se aritmeticamente impossibile). **WU229** potatura anticipata orfani nel pool (≥2 riletture tab senza report → morto, non aspetta il TTL). **Richiede riavvio BOT (WU227/228) + DASHBOARD (WU224/225/226/229)**. Dettagli in `docs/issues/telemetria-predictor.md` + `infra-startup.md`. **Miglioria futura identificata**: dare al greedy uno stato simulato per-istanza → il doppio giro diventa candidato ordinario dell'algoritmo (vedi WU228 note). |
...
| testing | — | Suite pytest completa — 143 fail su 1296 (debito pre-esistente) | BASSA | 🆕 SCOPERTA 22/07 lanciando `pytest -q` senza path (mai fatto prima nella pratica corrente — si usano sempre sottoinsiemi mirati). Verificato uno per uno: NON correlati a nessuna WU recente. (1) plugin `pytest-asyncio` mancante nell'ambiente → tutti i test scritti come `async def test_...` falliscono con "async def functions are not natively supported" (`test_task.py`, `test_rifornimento_base.py` e altri) — riguarda solo la firma del *test*, non la regola V6 "mai async def run" nel codice applicativo. (2) `test_ocr_helpers.py::test_zone_custom` — chiama `ocr_risorse(zone_risorse=...)` ma la funzione reale non ha più quel kwarg (drift test/codice). (3) 2 fail in `test_orchestrator.py` (`last_run` non aggiornato dopo eccezione, conteggio tick). Non indagato oltre — fuori scope della sessione che l'ha trovato (WU246, rollout event_center_claims). Da assegnare quando si vuole ripulire la suite completa. |
...
- Ogni modifica, bug, fix o attività rilevante effettuata sul progetto va
  **sempre notificata a Gemini** via `shared_ai_exchange/` (canale
  `channel.json` + `updates.md`, Protocollo v2 dal 23/07/2026 — v1 append-only
  archiviato in `shared_ai_exchange/archive/`), per mantenere allineamento
...
- **Prima di scrivere una richiesta/risposta a Gemini e prima di aggiornare
  ...
  `updates.md`) — evita di sovrascrivere un baton più recente. Regola
  esplicita dell'utente (17/07/2026), dettagli in `PROTOCOL.md` regola 2.
