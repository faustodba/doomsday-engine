...
| telemetria-predictor | WU225 | Ricostruzione `tempo_raccolta_dataset.jsonl` post-fix TTL | MEDIA | 🟡 PARZIALE 15/07 — il fix è **fatto** (`TTL_ORFANE_ORE` 4→12, effetto dal riavvio dashboard: il TTL potava le occupazioni prima che il report del completamento venisse letto, perché quello esiste solo al boot successivo dell'istanza — periodo di ciclo p50 3.46h. Curva misurata: 4h 506 match/768 orfani → **12h 1201/73**, satura a 16h). **Resta pending, non approvato**: ricostruire il dataset dai due sorgenti intatti (`nodi_mappa_observations.jsonl` + `report_raccolta_dataset.jsonl`, storia dal 10/07) per passare da 491 a ~1201 campioni **subito** invece di riaccumularli in giorni — rilevante perché WU223 Fase C ha eliminato lo statico e questo dataset è l'unica fonte di `_calc_t_marcia_min`, oggi distorto verso il basso dalla censura selettiva sui lenti (persi p50 3.00h/p90 3.89h vs matchati 2.82h/3.19h). Operazione che riscrive un file di dati → serve ok esplicito. Dettagli in `docs/issues/telemetria-predictor.md` |
