...
| telemetria-predictor | WU225 | TTL orfane 4h ≈ periodo di ciclo → 59% campioni tempo-raccolta persi, censura selettiva sui lenti | ALTA | 🆕 APERTA 15/07 — **diagnosi fatta, fix da concordare**. Il completamento raccolta esiste solo quando l'istanza riparte e legge il tab Report → latenza invio→report ≈ 1 periodo di ciclo (p50 **3.46h**, 29% ≥4h) mentre `TTL_ORFANE_ORE=4.0` pota prima. Misurato: 1269 report → 491 matchati, **706 orfani ma abbinabili** (660 con durata <4h) → cattura **41%**. Perdita **selettiva sui lenti** (persi p50 3.00h/p90 3.89h vs matchati 2.82h/3.19h) → dataset distorto verso il basso. Critico perché WU223 Fase C ha eliminato lo statico: è l'unica fonte di `_calc_t_marcia_min`. Non causato dai fermi bot (52-69% orfani anche nei giorni puliti). **Proposta**: TTL 4→8-10h (la soglia era tarata sulla durata di raccolta, ma la potatura misura l'età al run, che include l'attesa del boot successivo). Dettagli in `docs/issues/telemetria-predictor.md` |
