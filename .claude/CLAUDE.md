...
| telemetria-predictor | WU202 | Cutover predictor Fase B — T_marcia da misura empirica (flag-gated, tiered) | MEDIA | ✅ IMPLEMENTATA 12/07 (flag OFF, zero regressione). `_calc_t_marcia_min` tiered: empirico primario (cella ≥3 campioni) + fallback statico; flag `tempo_raccolta_empirico_enabled` (default OFF, DYNAMIC>STATIC). Piano/verifica/analisi eliminazioni in `docs/issues/predictor-cutover-plan.md`. Verifica prod: FAU_02 lento +15.6% vs statico, celle scarne fallback pulito. **Pending utente**: attivare shadow→LIVE pilota. Fase C (rimozione `t_marcia_calibration`) non iniziata |
